...
class phpCore
{
    ...
    public function __construct()
    {
        ...
        $nocookie = ($_SERVER["PHP_SELF"]!="/splash.php" && strpos($_SERVER["PHP_SELF"],'json') === false && strpos($_SERVER["PHP_SELF"],'html') === false && !isset($_COOKIE['mobile-splash']));
        if(!$this->isMobile()) 
        {
          if($nocookie)
            if($mobilebrowser)
          ...
          { 
            header("location: http://mobile.nerdz.eu".$_SERVER["REQUEST_URI"]);
          }
        }
        else
        {
          ...
          { 
            header("location: https://www.nerdz.eu".$_SERVER["REQUEST_URI"]);
          }
        }

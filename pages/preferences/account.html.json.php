...
if(
    db::NO_ERRNO != $core->query(array('UPDATE users SET "username" = :username, "timezone" = :timezone, "name" = :name, "surname" = :surname,"email" = :email,"gender" = :gender,
    "birth_date" = :date'. ($control ? ', "password" = ENCODE(DIGEST(:password, \'SHA1\'), \'HEX\')' : '') . ' WHERE counter = :id',$par),db::FETCH_ERRNO)
  )
    die($core->jsonResponse('error',$core->lang('ERROR')));
...
if($usernamechanged)
{
    ...
    if(db::NO_ERRNO != $core->query(array('INSERT INTO "posts" ("from","to","pid","message","notify", "time") VALUES ('.USERS_NEWS.','.USERS_NEWS.",{$lastpid}, :msg , FALSE, NOW())",array($message)),db::FETCH_ERRNO))
        die($core->jsonResponse('error',$core->lang('ERROR')));

...
switch(isset($_GET['action']) ? strtolower(trim($_GET['action'])) : '')
{
	case 'add':
		...
		if(!in_array($core->query(array('INSERT INTO "bookmarks"("from","hpid","time") VALUES(:to,:hpid,UNIX_TIMESTAMP())',array(':to' => $to, ':hpid' => $hpid)),db::FETCH_ERR),array(db::NO_ERR,1062)))
			die($core->jsonResponse('error',$core->lang('ERROR')));	
	break;
	case 'del':

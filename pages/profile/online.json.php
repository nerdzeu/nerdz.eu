...
{
    if(db::NO_ERR != $core->query(array('UPDATE "profiles" SET "remote_addr" = :addr WHERE "counter" = :id',array(':addr' => $_SERVER['REMOTE_ADDR'], ':id' => $_SESSION['nerdz_id'])),db::FETCH_ERR))
...
{
    if(db::NO_ERR != $core->query(array('UPDATE "profiles" SET "http_user_agent" = :uag WHERE "counter" = :id',array(':uag' => htmlentities($_SERVER['HTTP_USER_AGENT'],ENT_QUOTES,'UTF-8'), ':id' => $_SESSION['nerdz_id'])),db::FETCH_ERR))

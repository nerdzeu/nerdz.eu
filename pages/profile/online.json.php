...
if(!($o = $core->query(array('SELECT "remote_addr","http_user_agent" FROM "users" WHERE "counter" = :id',array(':id' => $_SESSION['nerdz_id'])),db::FETCH_OBJ)))
    die($core->jsonResponse('error',$core->lang('ERROR')));

if(empty($o->remote_addr) || empty($_SESSION['nerdz_remote_addr']) || ($o->remote_addr != $_SERVER['REMOTE_ADDR']))
{
    if(db::NO_ERRNO != $core->query(array('UPDATE "users" SET "remote_addr" = :addr WHERE "counter" = :id',array(':addr' => $_SERVER['REMOTE_ADDR'], ':id' => $_SESSION['nerdz_id'])),db::FETCH_ERRNO))
...
{
    if(db::NO_ERRNO != $core->query(array('UPDATE "users" SET "http_user_agent" = :uag WHERE "counter" = :id',array(':uag' => htmlspecialchars($_SERVER['HTTP_USER_AGENT'],ENT_QUOTES,'UTF-8'), ':id' => $_SESSION['nerdz_id'])),db::FETCH_ERRNO))

...
$query = empty($q) ?
		...
		array("SELECT name,surname,username, counter,birth_date,EXTRACT(EPOCH FROM last) AS last FROM users WHERE {$orderby} LIKE ? ORDER BY {$orderby} {$order} LIMIT {$limit}",array("%{$q}%"));
